...
// Signal handler for SIGINT and SIGTERM (graceful exit)
...
{
    syslog(LOG_DEBUG, "Starting transfer");
    ...
    int status = getaddrinfo(NULL, PORT, &sock , &addrinfo_s);  
    if (status != 0)
    {
    ...
    // socket creation
    sockfd = socket(sock.ai_family, sock.ai_socktype, 0);
    if (sockfd == -1)
    {
