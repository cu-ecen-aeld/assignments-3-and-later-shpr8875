...
static void signal_handler(int signal_number)
{
    if (signal_number == SIGINT || signal_number == SIGTERM)
    {
       ...
       /*
        if (fp)
        {
            fflush(fp);  // Make sure to flush before closing
            fclose(fp);
        }*/
        cleaup_slist();
        exit(0);
...
static bool handle_special_command(const char* command, int file_desc) {
    unsigned int write_cmd, write_cmd_offset;
    if (sscanf(command, "AESDCHAR_IOCSEEKTO:%u,%u", &write_cmd, &write_cmd_offset) == 2) {
        aesd_seekto seekto = {write_cmd, write_cmd_offset};

        // Ensure we are interacting with the right file descriptor
        if (ioctl(file_desc, AESDCHAR_IOCSEEKTO, &seekto) == -1) {
            syslog(LOG_ERR, "Failed ioctl AESDCHAR_IOCSEEKTO: %s", strerror(errno));
            return false;
        }

        syslog(LOG_DEBUG, "Performed ioctl seek to write_cmd: %u, write_cmd_offset: %u", write_cmd, write_cmd_offset);
        return true;
    }
...
static void connection_thread(void* thread) {
    ...
    while (threadInfo->conn_build == 0) {
        ...
        if (receiveResult == -1) {
            ...
            if (handle_special_command(receiveData, fileno(fp))) {
                // Special command processed, do not write to file
                syslog(LOG_DEBUG, "Handled special command: %s", receiveData);
            } else {
                // Write received data to the file
                fseek(fp, 0, SEEK_END);
                size_t written = fwrite(receiveData, sizeof(char), receiveResult, fp);
                if (written != receiveResult) {
                    syslog(LOG_ERR, "Failed to write full data. Expected: %d, Written: %zu", receiveResult, written);
                }
        ...
        // If newline found, send the file contents back to the client
        if (newlineFound) {
            threadInfo->conn_build = true;
...
void exit_procedure(void)
{
    ...
    remove(FILE_PATH);  // Clean up file if necessary

    cleaup_slist();  // Clean up the list of threads
    exit(-1);
}
